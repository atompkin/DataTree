...
asciinode * build_ascii_tree_recursive(struct Tree * t)
{
	...
	if (node->right != NULL)
	{
        ...
        if(t->op.length() == 0 && t->var == 0)
        {
            sprintf(node->label, "%.2f", t->val);
        }
        else if(t->op.length() == 0 && t->var != 0)
        {
            sprintf(node->label, "%c", t->var);
        }
        else
        {
            sprintf(node->label, "%s", t->op.c_str());
        }
